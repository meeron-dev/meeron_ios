    ...
    private func configureTextField() {
        ...
        meetingTitleTextField.rx.text.orEmpty
            ...
            .subscribe(onNext: { owner, text in
                if text.count > TextFieldLimitNumberConstant.meetingTitle{
                    owner.meetingTitleTextField.text = String(text.prefix(TextFieldLimitNumberConstant.meetingTitle))
                    owner.meetingTitleTextField.resignFirstResponder()
                }
                if text != "" {
                    owner.meetingTitleTextLimitLabelWidth.constant = 0
                    owner.meetingBaiscInfoCreationVM.saveTitle(title: text)
                }else{
                    owner.meetingTitleTextLimitLabelWidth.constant = 80
                    owner.meetingBaiscInfoCreationVM.saveTitle(title: "")
                }
            }).disposed(by: disposeBag)
        ...
        meetingPurposeTextField.rx.text.orEmpty
            ...
            .observe(on: MainScheduler.asyncInstance)
            .subscribe(onNext: { owner, text in
                if text.count > TextFieldLimitNumberConstant.meetingPurpose {
                    owner.meetingPurposeTextField.text = String(text.prefix(TextFieldLimitNumberConstant.meetingPurpose))
                    owner.meetingPurposeTextField.resignFirstResponder()
                }
                
                if text != "" {
                    owner.meetingPurposeTextLimitLabelWidth.constant = 0
                    owner.meetingBaiscInfoCreationVM.savePurpose(purpose: text)
                    
                }else{
                    owner.meetingPurposeTextLimitLabelWidth.constant = 80
                    owner.meetingBaiscInfoCreationVM.savePurpose(purpose: "")
                }
            }).disposed(by: disposeBag)
